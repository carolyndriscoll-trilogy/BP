...
export async function summarizeFact(fullText: string): Promise<string> {
  ...
  try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a concise fact summarizer. Summarize the provided text into a maximum of 3 lines. Be direct and clear. Do not use fluff. Output must be plain text. No markdown.",
            },
            {
              role: "user",
    ...
    return (
      data.choices[0]?.message?.content?.trim() || fullText.substring(0, 200)
    );
  } catch (error) {
